...
class NoiseGenerator {
  ...
  perlinNoise(x, z) {
    ...
    for (let i = 0; i <= this.configs.octaves; i++) {
      ...
      const amplitude = Math.pow(this.persistance, i);
      const noise = this.noise(
        (x * frequency) / this.configs.smoothness,
        (z * frequency) / this.configs.smoothness
      );
      r += noise + amplitude;
    }
    const result = (r / 2 + 1) * this.amplitude - 20;
